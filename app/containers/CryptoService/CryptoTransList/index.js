...
export function CryptoTransList() {
  ...
  const getData = async fiterData => {
    ...
    await socket.emit(SOCKET_GET_CRYPTO_TRANSACTION_LIST, {
      data: fiterData,
    });
    await socket
      .off(SOCKET_GET_CRYPTO_TRANSACTION_LIST)
      .on(SOCKET_GET_CRYPTO_TRANSACTION_LIST, res => {
        setLoading(false);
        const resParsed = JSON.parse(res);
        if (resParsed.result) {
  ...
  return (
    <div>
      <Helmet>
        <title>Crypto Transaction List</title>
      </Helmet>
...
export default CryptoTransList;
