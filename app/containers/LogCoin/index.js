...
import { SOCKET_GET_COIN_LOG } from './constants';
...
export function LogCoin() {
  ...
  const fetchData = async fiterData => {
    setLoading(true);
    await socket.emit(SOCKET_GET_COIN_LOG, { data: fiterData });
    await socket.on(SOCKET_GET_COIN_LOG, res => {
      setLoading(false);
      const resParsed = JSON.parse(res);
      if (resParsed.result) {
        console.log('SOCKET_GET_COIN_LOG', resParsed.data);
