...
import {
  SOCKET_GET_LOG_SHOOT_FISH_JACPOT,
  SOCKET_GET_ROOM_LIST,
} from './constants';
...
export function LogShootFishJackpot() {
  ...
  const fetchData = async fiterData => {
    setLoading(true);
    await socket.emit(SOCKET_GET_LOG_SHOOT_FISH_JACPOT, { data: fiterData });
    await socket.on(SOCKET_GET_LOG_SHOOT_FISH_JACPOT, res => {
      setLoading(false);
      const resParsed = JSON.parse(res);
      if (resParsed.result) {
        console.log('SOCKET_GET_LOG_SHOOT_FISH_JACPOT', resParsed.data);
  ...
  return (
    <div>
      <Helmet>
        <title>Log Shoot Fish Jackpot</title>
      </Helmet>
      <div className="page-header-wrapper">
        <PageHeader
          style={{ paddingLeft: '0', paddingRight: '0' }}
          className="site-page-header"
          title="Log Shoot Fish Jackpot"
        />
      </div>
...
export default LogShootFishJackpot;
